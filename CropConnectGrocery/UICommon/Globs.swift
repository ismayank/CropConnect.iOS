...
// Globs class to store constant values such as URLs and keys
struct Globs {
    ...
    static let userLogin = "user_login"  // This will be used to store the access token
...
// KKey struct to store standard response keys
struct KKey {
...
// Utils class for UserDefaults operations and managing the access token
class Utils {
    
    // Save any data to UserDefaults
    class func UDSET(data: Any, key: String) {
        UserDefaults.standard.set(data, forKey: key)
        UserDefaults.standard.synchronize()
    }
    
    // Retrieve any data from UserDefaults
    ...
    class func saveAccessToken(token: String) {
        UDSET(data: token, key: Globs.userLogin) // Use a unique key to store the access token
    }
    
    // Retrieve access token from UserDefaults
    class func getAccessToken() -> String? {
        return UDValue(key: Globs.userLogin) as? String
    }
    
    // Remove access token from UserDefaults (log out)
    class func removeAccessToken() {
        UDRemove(key: Globs.userLogin)
    }
}
